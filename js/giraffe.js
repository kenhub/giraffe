...
var auth, changeDashboard, createGraph, dashboard, dataPoll, default_period, description, generateDataURL, graphScaffold, graphs, init, metrics, period, refresh, refreshSummary, refreshTimer, scheme, toggleCss, _avg, _formatBase1024KMGTP, _max, _min, _sum;
...
generateDataURL = function(target, annotator_target) {
  if (typeof target === "function") {
    target = target();
  }
  annotator_target = annotator_target ? "&target=" + annotator_target : "";
  return "" + graphite_url + "/render?from=-" + period + "minutes&target=" + target + annotator_target + "&format=json&jsonp=?";
};
...
Rickshaw.Graph.JSONP.Graphite = Rickshaw.Class.create(Rickshaw.Graph.JSONP, {
  ...
  getAjaxData: function(period) {
    ...
    return deferred = $.ajax({
      dataType: 'json',
      url: generateDataURL(this.args.target, this.args.annotator_target),
