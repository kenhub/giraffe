...
(function() {
  var auth, changeDashboard, createGraph, dashboard, dataPoll, default_period, description, generateDataURL, generateEventsURL, generateGraphiteTargets, getTargetColor, graphScaffold, graphs, init, instance_url, metrics, period, refresh, refreshSummary, refreshTimer, scheme, toggleCss, _avg, _formatBase1024KMGTP, _last, _max, _min, _sum;
  ...
  init = function() {
    var dash, i, instance, metric, refreshInterval, _i, _j, _k, _len, _len1, _len2;
    ...
    if (graphite_url instanceof Array) {
      ...
      for (_j = 0, _len1 = graphite_url.length; _j < _len1; _j++) {
        instance = graphite_url[_j];
  ...
  generateDataURL = function(targets, annotator_target) {
    ...
    return "" + instance_url + "/render?from=-" + period + "minutes&" + data_targets + annotator_target + "&format=json&jsonp=?";
  };
  ...
  generateEventsURL = function(event_tags) {
    ...
    return "" + instance_url + "/events/get_data?from=-" + period + "minutes" + tags + jsonp;
  };
  ...
  */
  ...
  $('#dropdown-dash').on('click', 'a', function() {
  ...
  if (graphite_url instanceof Array) {
    $('#dropdown-instance').on('click', 'a', function() {
      var instance;

      instance = _.where(graphite_url, {
        name: $(this).text()
      })[0];
